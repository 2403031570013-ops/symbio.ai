...
async def get_dynamic_pricing(
    ...
    pricing = await _run(DynamicPricing.find(DynamicPricing.material_id == material_id).sort(-DynamicPricing.created_at).to_list())
    return pricing
...
async def create_smart_notification(
    ...
    await _run(db_notification.insert())
...
async def get_smart_notifications(
    ...
    notifications = await _run(SmartNotification.find(SmartNotification.user_id == current_user.id).sort(-SmartNotification.created_at).skip(skip).limit(limit).to_list())
...
async def mark_notification_read(
    ...
    notification = await _run(SmartNotification.find_one(SmartNotification.id == notification_id, SmartNotification.user_id == current_user.id))
    if not notification:
...
async def create_workflow_automation(
    ...
    await _run(db_automation.insert())
    return db_automation


@router.get("/workflow-automation", response_model=List[WorkflowAutomationResponse])
async def get_workflow_automations(
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user)
):
    """Get workflow automations"""
    automations = await _run(WorkflowAutomation.find(WorkflowAutomation.created_by == current_user.id).to_list())
    return automations
...
async def get_contracts(
    ...
    contracts = await _run(Contract.find(Contract.party_a_id == factory_id).sort(-Contract.created_at).to_list())
    return contracts
...
async def get_payments(
    ...
    payments = await _run(Payment.find((Payment.paid_by == factory_id) | (Payment.paid_to == factory_id)).sort(-Payment.created_at).to_list())
    return payments
...
async def get_business_intelligence(
    ...
    reports = await _run(BusinessIntelligence.find(BusinessIntelligence.report_type == report_type).sort(-BusinessIntelligence.created_at).limit(10).to_list())
    return reports
...
async def get_anomaly_detections(
    ...
    detections = await _run(AnomalyDetection.find(AnomalyDetection.entity_id == factory_id).sort(-AnomalyDetection.created_at).to_list())
    return detections
...
async def get_predictive_maintenance(
    ...
    maintenance = await _run(PredictiveMaintenance.find(PredictiveMaintenance.factory_id == factory_id).sort(-PredictiveMaintenance.updated_at).to_list())
    return maintenance
